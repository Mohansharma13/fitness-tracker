...
export async function createWorkoutTemplateFromWorkout(db: SQLiteDatabase, workout: WorkoutLog, name: string): Promise<void> {
  ...
  const now = new Date().toISOString();
  await db.withTransactionAsync(async () => {
    await db.runAsync('INSERT INTO workout_templates (id, user_id, name, created_at, updated_at, deleted_at, sync_status) VALUES (?, ?, ?, ?, ?, ?, ?)', id, null, name.trim() || workout.workoutType, now, now, null, 'local');
...
export async function applyWorkoutTemplate(db: SQLiteDatabase, templateId: string, date: string): Promise<void> {
  ...
  for (const exercise of exercises) await createExerciseLog(db, { workoutLogId: workout.id, exerciseName: exercise.exercise_name, sets: exercise.sets, reps: exercise.reps, weight: exercise.weight });
}
